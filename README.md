...
サウンドルーティングシステムの構成図および設定情報をまとめたリポジトリです。

## システム構成図

![Sound Routing System](doc/system.svg)

## 概要

RME Babyface Pro FS をオーディオインターフェースのコアに据え、配信・収録・モニタリングを目的としたサウンドルーティングを構築しています。Reaper を中心に、各種ソフトウェア入出力（Discord、Notion など）とハードウェア入出力を柔軟にルーティングします。

## ハードウェア構成

| 種別 | 機材 | 用途 |
|------|------|------|
| オーディオインターフェース | RME Babyface Pro FS | サウンドルーティングのコア |
...
構成図は [draw.io](https://app.diagrams.net/) で編集できます。`doc/system.drawio` を開いて編集後、SVG / PNG 形式でエクスポートしてください。
